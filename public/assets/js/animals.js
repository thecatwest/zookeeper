...
const getAnimals = (formData = {}) => {
  ...
  fetch(queryUrl)
    .then(response => {
      // check to see if okay property in response is true or false
      if(!response.ok) {
        return alert('Error: ' + response.statusText);
      }
      // use .json() method to parse response into readable JSON format
      return response.json();
    })
    .then(animalData => {
      console.log(animalData);
      // send array of animal data to printResults() function, where generates cards for each animal and prints them to page
